RSpec.describe HCL::Checker do
  context 'list of complex objects ' do
    ...
    it 'accepts multiples of common type hcl elements' do
      ...
      ret = HCL::Checker.parse hcl_string
      expect(ret).to eq({"template"=>[{"source"=>"/etc/vault/server.key.ctmpl", "destination"=>"/etc/vault/server.key"},{"source"=>"/etc/vault/server.crt.ctmpl", "destination"=>"/etc/vault/server.crt"}]})
    end
      ...
      expect(ret).to eq({
        "auto_auth"=>
          ...
          "sink"=>[
            ...
            {"file"=>{"wrap_ttl"=>"5m", "aad_env_var"=>"TEST_AAD_ENV", "dh_type"=>"curve25519", "dh_path"=>"/tmp/file-foo-dhpath2", "config"=>{"path"=>"/tmp/file-bar"}}}
          ]
        }
      })
    end
